...
export default function Footer() {
  return (
    <footer className="bg-card border-t border-white/10 py-14 relative overflow-hidden">
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        <div className="flex flex-col md:flex-row items-start justify-between gap-10">
          {/* Left — social links */}
          <ul className="flex flex-col gap-3 font-body text-sm text-muted-foreground">
            {socials.map((social) => (
              <li key={social.name}>
                <a
                  href={social.href}
          ...
          {/* Right — nav links */}
          <ul className="flex flex-col gap-3 font-body text-sm text-muted-foreground md:text-right md:items-end">
            {quickLinks.map((link) => (
              <li key={link.label}>
                <Link
                  to={link.href}
